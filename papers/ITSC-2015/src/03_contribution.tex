\section{Holistic parametric approach}
...
%Simply put, if a car's position matches a charging station's position while the car isn't driving, the power load of the car gets assigned to the power load of the charging station.
...
%Route and speed selection are determined non-deterministically based on the current position of the car. 
...
%Furthermore, the route to the destination is dependent on the cars current state of charge. 
...
The objectives of a car's behavior are represented through multiple cost functions. Generally, costs of a car can only incur when it's departure time has been reached and are aggregated over time. %Subsequently, we describe the underlying cost functions in detail. 
