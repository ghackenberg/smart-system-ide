...
\end{itemize}
Note that the previous parameters completely determine our road segment model. Consequently, we abstract from a variety of parameters typically considered such as continuous elevation profiles~\cite{?}.
...
\begin{itemize}
	...
	\]
\end{itemize}
Note that we again abstract from many parameters typically considered such as the vehicle weight~\cite{?} or the exact vehicle geometry~\cite{?}. In particular, we approximate mechanical and electrical efficiencies with constants only.
