...
import org.xtream.demo.hydro.data.PolynomLevelDelta;
import org.xtream.demo.hydro.model.Constants;
import org.xtream.demo.hydro.model.context.reactive.VolumeComponent;

public class VolumeDeltaComponent extends VolumeComponent
{
	
	// Parameters
	
	private PolynomLevelDelta model;
	...
	public Expression<Double> levelExpression = new Expression<Double>(levelOutput, true)
	{
		@Override protected Double evaluate(State state, int timepoint)
		{
			...
			else
			{
				...
				return levelOutput.get(state, timepoint - 1) + model.estimate(inflows, outflows);
			}
		}
	};
