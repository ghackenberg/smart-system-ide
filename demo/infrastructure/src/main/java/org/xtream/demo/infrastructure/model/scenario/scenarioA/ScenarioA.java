...
public class ScenarioA extends Scenario {
	 ...
	 * SCENARIO: ScenarioA
	 * 
	 * PURPOSE: Show single car with low energy driving to destination
	 * 
	 * WEIGHTS: Cars: SOC: 1
	 * 				  Time: 15
	 * 				  Energy: 1
	 ...
	 * 				  Med: 1.
	 * 
	 * RANDOMNESS: chargingStations and their net allocation (based on KMeans)
	 * 			   random net allocation of specific producers and consumers of PowerSystem
	 * 
	 * OBSERVATION: Low charging affinity, recharging at charging stations on route when threshold is reached
	 * 
	 */
	
	public static int numberCars = 1;
	...
	public TransportationComponent createTransportationComponent(PowerComponent powerComponent) 
	{
		...
		{
			carSet.add(carFactory.generateCar(context.getEdge("OriginA"), context.getEdge("DestinationA"), Car.LOWENERGYCAR));		
		}
	...
	public PowerComponent createPowerComponent() 
	{
		...
		{
			lowVoltageNets[i] = new NetComponent((numberBatteries+numberSolarPanels+numberChargingStations)/numberLowVoltageNets, 1.);
		}
		
		for (int i = 0; i < numberMediumVoltageNets; i++)
		{
			mediumVoltageNets[i] = new NetComponent(numberLowVoltageNets/numberMediumVoltageNets, 1.);
		}
