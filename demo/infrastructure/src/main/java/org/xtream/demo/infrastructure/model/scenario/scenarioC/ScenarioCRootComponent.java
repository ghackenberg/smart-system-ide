package org.xtream.demo.infrastructure.model.scenario.scenarioC;
...
public class ScenarioCRootComponent extends RootComponent {
	
	public ScenarioCRootComponent() 
	{
		super(new ScenarioC());
	}
	...
	{
		new Workbench<>(new Engine<>(new ScenarioCRootComponent(), Scenario.SAMPLES, Scenario.CLUSTERS, Scenario.BRANCH_ROUNDS, Scenario.BRANCH_DURATION, true, new GridStrategy()), Scenario.DURATION, new ComponentHierarchyPart<>(0,0,1,2), new ComponentChildrenPart<>(0,2,1,2), new ViolationChartMonitorPart<>(3,0), new ModelScenePart<>(1,2,2,2), new ComponentChartsPart<>(3,2,2,2), new StatesChartMonitorPart<>(5,0), new ClustersChartMonitorPart<>(5,1), new TracesChartMonitorPart<>(3,1), new StateSpacePart<>(3,1), new ObjectiveChartMonitorPart<>(4,1), new TimeChartMonitorPart<>(5,2), new MemoryChartMonitorPart<>(5,3));
	}
